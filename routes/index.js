...
var itemModel = require('../modules/item');
var uploadModel = require('../modules/upload');

var router = express.Router();
var item =itemModel.find({});
...
if(err) throw err;
res.render('index', { title: 'Item Records', records:data, success:'' });
  });
...
router.post('/', upload, function(req, res, next) {
  var empDetails = new itemModel({
    ItemName: req.body.uname,
    tags: req.body.tags,
...
router.post('/search/', function(req, res, next) {
  ...
  var employeeFilter =itemModel.find(flterParameter);
  employeeFilter.exec(function(err,data){
      if(err) throw err;
      res.render('index', { title: 'Item Records', records:data });
        });
...
var del= itemModel.findByIdAndDelete(id);

del.exec(function(err){
if(err) throw err;
item.exec(function(err,data){
  if(err) throw err;
  res.render('index', { title: 'Item Records', records:data, success:'Record Deleted Successfully' });
    });
  });
...
router.get('/edit/:id', function(req, res, next) {
  var id=req.params.id;
var edit= itemModel.findById(id);
edit.exec(function(err,data){
if(err) throw err;
res.render('edit', { title: 'Edit Item Record', records:data });
  });
...
var update= itemModel.findByIdAndUpdate(req.body.id,dataRecords);
...
router.get('/autocomplete/', function(req, res, next) {
  ...
  var employeeFilter =itemModel.find({name:regex},{'name':1}).sort({"updated_at":-1}).sort({"created_at":-1}).limit(20);
  employeeFilter.exec(function(err,data){
